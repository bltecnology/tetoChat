...
const ModalContacts = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    const sanitizeField = (field) => (field.trim() === '' ? null : field);

    const newContact = {
      name: sanitizeField(name),
      phone: sanitizeField(phone),
      tag: sanitizeField(tags),
      note: sanitizeField(observation),
      cpf: sanitizeField(cpf),
      rg: sanitizeField(rg),
      email: sanitizeField(email),
    };

    try {
      await axios.post('https://tetochat-backend.onrender.com/contacts', newContact);
      ...
      <div className="bg-white p-4 rounded-lg w-1/2">
        ...
        <div className="mb-2">
          ...
          <input
            type="text"
            value={phone}
            placeholder="O código do país é obrigatório"
