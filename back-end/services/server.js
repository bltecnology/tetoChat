  ...
  app.delete('/contacts/:id', authenticateJWT, async (req, res) => {
    ...
    try {
      // Excluir todas as mensagens associadas ao contato
      await pool.query("DELETE FROM whatsapp_messages WHERE contact_id = ?", [contactId]);
      ...
      console.error('Erro ao deletar contato:', error);
      res.status(500).send(`Erro ao deletar contato: ${error.message}`);
    }
  });
